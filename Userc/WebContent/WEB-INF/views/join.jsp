...
function registerCheckFn(){
	...
	$.ajax({
		...
		success : function(result) {
			if(result==1){
				$('#checkMessage').html("사용할수있는 아이디입니다.");
				$('#checkType').attr('class','modal-content panel-success');
			}else{
				$('#checkMessage').html("사용할수없는 아이디입니다.");
...
<body>
  ...
  <li class="active"><a href="join.do">회원가입</a>
  </ul>
  </li>
  </ul>
  <%
  ...
  <td><input class="form-control" type="text" id="userID" name="userID" maxlength="20" placeholder="아이디를 입력하세요"></td>
   <td style="width: 110px"><button class="btn btn-primary" onclick="registerCheckFn()" type="button">중복체크</button></td>
  </tr>
  <tr>
